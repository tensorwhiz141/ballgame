"""
Blink-controlled jumping ball game that can work with either:
1. Simulated EEG data (for testing)
2. Real EEG data from Raspberry Pi Pico
"""
...
from pico_eeg_reader import PicoEEGReader
...
class GameWithPico:
    def __init__(self, use_pico=False, pico_port='COM3'):
        ...
        # EEG data source
        self.use_pico = use_pico
        if use_pico:
            print(f"Using real Pico data from {pico_port}")
            self.eeg_source = PicoEEGReader(port=pico_port)
        else:
            print("Using simulated EEG data")
            self.eeg_source = EEGSimulator(sampling_rate=100)
        
        # Blink detection
        self.blink_detector = RealTimeBlinkDetector(self.eeg_source)
    ...
    def update_eeg_visualization(self):
        ...
        # Get recent EEG data
        timestamps, values = self.eeg_source.get_buffer_data(self.max_eeg_points)
    ...
    def draw_eeg_visualization(self):
        ...
        # Label and status
        source_label = "Real Pico Data" if self.use_pico else "Simulated Data"
        label = self.small_font.render(f"EEG Signal ({source_label})", True, WHITE)
        self.screen.blit(label, (eeg_x, eeg_y - 25))
        
        # Connection status
        status_color = GREEN if self.connection_status == "Connected" else RED
        status_text = self.small_font.render(f"Status: {self.connection_status}", True, status_color)
        self.screen.blit(status_text, (eeg_x + 150, eeg_y - 25))
    ...
    def draw_ui(self):
        ...
        instructions = [
            ...
            "Press B to trigger test blink (sim only)",
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_q:
                    ...
                    self.reset_game()
                elif event.key == pygame.K_b and not self.use_pico:  # Only for simulator
                    self.eeg_source.trigger_blink()
    ...
    def run(self):
        ...
        if not self.use_pico:
            print("- Press B to trigger a test blink (simulation mode)")
        print("- Press P to pause, R to restart, Q to quit")
        
        # Start EEG data source and blink detection
        if self.blink_detector.start():
            self.connection_status = "Connected"
        else:
            self.connection_status = "Failed to connect"
            print("Warning: Could not start EEG data source")
...
def main():
    ...
    parser.add_argument("--pico", action="store_true", help="Use real Pico data instead of simulation")
    parser.add_argument("--port", default="COM3", help="Serial port for Pico (default: COM3)")
    
    args = parser.parse_args()
    
    if args.pico:
        print(f"Starting game with real Pico data from {args.port}")
        print("Make sure your Pico is connected and running the EEG reader code!")
    else:
        print("Starting game with simulated EEG data")
    
    game = GameWithPico(use_pico=args.pico, pico_port=args.port)
    game.run()
